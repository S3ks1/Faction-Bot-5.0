            ...
            switch(commandName){
                ...
                case "flick": case "lever":
                    let levers = bot.findBlock({
                       ...
                       bot.chat(`${person.ign}, I flicked the lever at ${leverpos} to the ${levers.metadata == 6 ? "On Position" : "Off Position"}`)
                    break;
                case "help":
                ...
                case "volume":
                    ...
                    if(!server_queuea){
                        ...
                        bot.chat(` (!) Set volume of the current music to ${args[0]}`)
                    }
...
client.on('message', async (message) => {
    ...
    if(commandName === "help"){
        let names = [
            "help - help - List all of the commands loaded by the bot",
            "eval - eval <code> - Evaluate javascript code",
            "exec - exec <code> - Execute code through terminal",
            "snipe - snipe - Send the last deleted message", 
            "av - av [user] - Get a user's avatar",
            "whitelist - whitelist [remove/list/clear] - Manage the bot's ingame whitelist",
            "flist - flist - Get the current Faction List",
            "ftop - ftop - Get the current Faction Top",
            "fwho - fwho [faction] - Get a faction's f who",
            "sudo - sudo /<command> - Run a command or send a message ingame",
            "runcmd - runcmd <command> - Run a command ingame, automatically using a /",
            "wtop - wtop - Get the wall leaderboard",
            "btop - btop - Get the buffer leaderboard",
            "rtop - rtop - Get the rpost leaderboard",
            "settings - settings [key] [value] - Change bot settings",
            "members - members [role ID/name/mention] - Get a list of members with a specific role",
        ...
        let names2 = [
            ...
            "setstats - setstats <discord user/IGN> <walls/buffers/rpost> <number> - Set a user's ingame statistics",
            "tts - Text to Speech - Text to Speech",
            "ttsvoice - ttsvoice [voice name(case sensitive)] - Change the voice you use for text to speech",
            "tps - tps - Get the current TPS of the server",
            "rpost - rpost - Get ingame raidingoutpost status",
            "walls - walls - Get the last wall check information",
            "buffers - buffers - Get the last buffer check information",
            "raidingoutpost - raidingoutpost - Get the last rpost check information",
            "status - status - Get overall status for the guild",
            "tempgrace - tempgrace <time> - Add a temporary grace period to wall checks",
            "serverinfo - serverinfo - Get information about the current guild",
            "whois - whois [discord user] - Get information about a discord user",
            "calc - calc <calculation> - Perform a calculation",
            "play - play <song> - Play a song",
            "skip - skip - Skip the current song",
            "stop - stop - Stop all music",
 ...
 await message.author.send(ingame.join("\n")).catch((err) => {
    ...
    if(commandName == "tempgrace"){
        if(!args[0]){
            ...
            message.channel.send(embed)
            bot.chat(`(!) Grace has been enabled for ${ms(ms(args[0]))}!`)
            guild.grace = true
            guild.save().then((s) => {
                setTimeout(() => {
                    guild.grace = false
